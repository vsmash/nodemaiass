...
- bump vite/postcss/nanoid to clear Dependabot alerts, add dependabot.yml (#31)
-   - npm audit fix: vite 8.0.8 to 8.2.2, postcss 8.5.10 to 8.5.26, nanoid to 3.3.18
	  - clears 5 Dependabot alerts (#23 #24 #25 #26 #29), all dev-only transitive deps under vitest
	  - npm audit 0 vulnerabilities, was 3 high; 154/154 unit tests pass on Node 20/22/24
	  - no package.json change; published package ships chalk/dotenv/yargs only, so no end-user exposure
	  - add .github/dependabot.yml: groups prod and dev minor+patch updates weekly, majors stay ungrouped for individual review
	  - target-branch develop keeps version-update PRs off promote-only main
	  - documented: target-branch does not apply to security updates, which always target the default branch and cannot be grouped
	  - documented: Dependabot reads this file from the default branch, so the config is inert until develop is promoted to main
	  - supersedes Dependabot PR #27, which bumped vite alone and left postcss and nanoid unpatched
	Bumped version to 5.15.13
...
- auto-stage off by default + rename -ac→-uc; stash/restore dirty-tree -a (#23)
