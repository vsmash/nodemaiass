 ...
 */
export { isAIModeActive, parseChangelog, groupByMinor, buildVersionCommitMap };
...
async function cleanupOneFile(filePath, style, versionMap) {
  ...
  for (const group of groups) {
    ...
    cleanedGroups.push(cleaned.trim());
...
export async function handleCleanupCommand() {
  ...
  // Commit the cleanup on develop (only if there are tracked-file changes
  // beyond the gitignored .bak files).
  const status = gitOK(['status', '--porcelain', '--', publicFile, internalFile]);
  const hasChanges = status.ok && status.stdout !== '';
  if (hasChanges) {
    const filesToAdd = [];
    if (publicExists) filesToAdd.push(publicFile);
    if (internalExists) filesToAdd.push(internalFile);
    const add = gitOK(['add', ...filesToAdd]);
    if (!add.ok) {
