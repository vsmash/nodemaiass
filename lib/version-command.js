...
/**
 * Flags accepted by `maiass version`.
 * Consumed by the global flag validator in maiass.mjs so that subcommand-
 * specific flags are not rejected as "unrecognized" before reaching the
 * handler. See MAI-43.
 */
export const FLAGS = [
  '--dry-run',
...
export async function handleVersionCommand(args) {
  ...
  let positionalArgs, dryRun, tag, tagMessage, force;
  ...
  if (Array.isArray(args)) {
    ...
    ({ _: positionalArgs, 'dry-run': dryRun, tag, 'tag-message': tagMessage, force } = args);
