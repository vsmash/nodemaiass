...
function displayVersionInfo(versionInfo) {
  console.log(colors.BCyan(`${SYMBOLS.INFO} Version Information:`));
  console.log();
  ...
  if (versionInfo.files.length > 0) {
    console.log(colors.BCyan(`${SYMBOLS.INFO} Version Files Found:`));
    console.log();
...
function displayBumpPreview(currentVersion, newVersion, bumpType, files) {
  console.log(colors.BCyan(`${SYMBOLS.INFO} Version Bump Preview:`));
  console.log();
  ...
  if (files.length > 0) {
    console.log(colors.BCyan(`${SYMBOLS.INFO} Files to Update:`));
    console.log();
...
async function handleVersionBump(bumpType, options) {
  ...
  console.log(colors.BCyan(`${SYMBOLS.INFO} ${dryRun ? 'DRY RUN: ' : ''}Version Bump Operation`));
  console.log();
  ...
  if (!versionInfo.current && !force) {
    ...
    console.log();
    console.log(colors.BYellow(`${SYMBOLS.INFO} Use --force to create initial version`));
    return;
  }
  ...
  if (!newVersion) {
    ...
    console.log();
    console.log(colors.BYellow(`${SYMBOLS.INFO} Valid types: major, minor, patch, or specific version (e.g., 2.1.0)`));
    return;
  }
  ...
  if (!dryRun && !force) {
    ...
    if (confirm === 'n') {
      console.log(colors.BYellow(`${SYMBOLS.INFO} Version bump cancelled`));
      return;
    }
  }
  ...
  if (versionInfo.files.length > 0) {
    console.log(colors.BBlue(`${SYMBOLS.INFO} ${dryRun ? 'Would update' : 'Updating'} version files...`));
    
    const updateResult = await updateVersionFiles(newVersion, versionInfo.files, dryRun);
    
    if (updateResult.success) {
      updateResult.updated.forEach(update => {
        console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} ${update.file} ${colors.Gray(`(${update.oldVersion} → ${update.newVersion})`)}`));
      });
      
      if (updateResult.failed.length > 0) {
        console.log();
        console.log(colors.BYellow(`${SYMBOLS.WARNING} Some files failed to update:`));
    ...
    console.log(colors.BYellow(`${SYMBOLS.WARNING} No version files to update`));
  }
  
  // Create git tag if requested
  if (tag && !dryRun) {
    console.log();
    console.log(colors.BBlue(`${SYMBOLS.INFO} Creating git tag...`));
    
    const tagResult = await createVersionTag(newVersion, tagMessage);
    
    if (tagResult.success) {
      console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Created tag: ${newVersion}`));
    ...
    console.log();
    console.log(colors.BBlue(`${SYMBOLS.INFO} Would create git tag: ${newVersion}`));
    if (tagMessage) {
  ...
  console.log();
  if (dryRun) {
    console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Dry run completed successfully`));
    console.log(colors.BBlue(`${SYMBOLS.INFO} Run without --dry-run to apply changes`));
  } else {
    console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Version bump completed: ${versionInfo.current || '(none)'} → ${newVersion}`));
  }
}
