...
export function generateMachineFingerprint() {
  try {
    // Get components in same order as bashmaiass
    const mac = getMacAddress();
    const cpu = getCpuInfo();
    const disk = getDiskIdentifier();
    const kernel = getKernelInfo();
    
    // Create fingerprint input string: MAC|CPU|Disk|Kernel
    const fingerprintInput = `${mac}|${cpu}|${disk}|${kernel}`;
    
    // Generate SHA-256 hash in hex format (not base64) to match bashmaiass
