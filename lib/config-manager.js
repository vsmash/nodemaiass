...
export function getConfigPaths() {
  return {
    global: path.join(os.homedir(), '.maiass.env'),
    project: path.resolve(process.cwd(), '.env'),
...
export function writeConfig(configPath, config, options = {}) {
  ...
  try {
    ...
    const categories = {
      ...
      'OpenAI': ['MAIASS_OPENAI_MODE', 'MAIASS_OPENAI_TOKEN', 'MAIASS_OPENAI_MODEL', 'MAIASS_OPENAI_TEMPERATURE', 'MAIASS_OPENAI_ENDPOINT', 'MAIASS_OPENAI_MAX_CHARACTERS', 'MAIASS_OPENAI_COMMIT_MESSAGE_STYLE'],
...
export function editConfig(configPath) {
  try {
    ...
    console.log(colors.BBlue(`${SYMBOLS.INFO} Opening ${configPath} in ${editor}...`));
    ...
    console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Configuration file updated`));
    
  } catch (error) {
    console.error(colors.Red(`${SYMBOLS.CROSS} Error opening editor: ${error.message}`));
    console.log(colors.BYellow(`${SYMBOLS.INFO} You can manually edit: ${configPath}`));
  }
}
...
export function validateConfig(config) {
  ...
  Object.entries(config).forEach(([key, value]) => {
    ...
    if (key === 'MAIASS_OPENAI_TEMPERATURE') {
