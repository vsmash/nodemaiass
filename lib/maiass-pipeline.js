...
async function handleMergeToDevelop(branchInfo, commitResult, options = {}) {
  ...
  logger.header(SYMBOLS.INFO, 'Merge to Develop Phase');
  ...
  logger.info(`Merging ${currentBranch} into ${developBranch}...`);
...
export async function runMaiassPipeline(options = {}) {
  ...
  try {
    ...
    log.blue(SYMBOLS.INFO, 'Phase 1: Branch Detection and Validation');
    ...
    logger.info(`Phase 3: Merge to Develop`);
    ...
    logger.info(`Phase 4: Version Management`);
