...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    const internalLogResult = executeGitCommand(`git log "${lastTag}"..HEAD --pretty=format:"(%an) %B"`, true);
    if (!internalLogResult.success || !internalLogResult.output.trim()) {
      console.log(colors.BYellow(`${SYMBOLS.INFO} No commits found since last release`));
      return;
    }
    // extract %an from internalLogResult.output
    const authorNames = internalLogResult.output.split('\n').map(line => line.split(')')[0].replace('(', ''));
    // mainLogResult is internalLogResult without %an
    const mainLogResult = internalLogResult.output.split('\n').map(line => line.split(')')[1]);

    // Process main changelog commits (filter irrelevant + strip JIRA tickets)
    const commitMessages = internalLogResult.output.split('\n').map(line => line.split(')')[1])
