  ...
  try {
    ...
    const internalCommitMessages = internalLogResult.success ? internalLogResult.output
      ...
      .filter(line => {
        ...
        // Format is: "hash subject (author)" so check if subject contains merge/bump keywords
        return !(/\b(ncl|merge|bump|bumping|fixing merge conflicts|version bump|merged)\b/i.test(line));
      })
      .filter(line => line.length > 0) : [];
