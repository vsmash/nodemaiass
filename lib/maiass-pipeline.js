...
async function handleMergeToDevelop(branchInfo, commitResult, options = {}) {
  ...
  // -a (commit-only auto mode): stop here. Commit + push has already happened
  // in the commit phase. No merge, no version bump, no browser.
  if (process.env.MAIASS_AUTO_FINISH_AFTER_COMMIT === 'true') {
    log.blue(SYMBOLS.INFO, `Auto mode (-a) — feature branch committed and pushed; stopping before merge`);
    return { success: true, autoFinish: true };
  }

  // -am or interactive: this is auto-merge mode. autoMerge=true means "skip
  // prompts but still do the action" — including opening a PR browser tab if
