...
/**
 * Update changelog with commit messages since last release tag
 * Matches bash script logic: collect commits, filter irrelevant ones, format properly
...
async function updateChangelog(newVersion) {
  ...
  try {
    ...
    let lastTag;
    try {
      const tagResult = executeGitCommand('git describe --tags --abbrev=0', true);
      lastTag = tagResult.success ? tagResult.output.trim() : null;
    } catch (error) {
      lastTag = null;
    }
    ...
    const gitLogCommand = lastTag 
      ? `git log "${lastTag}"..HEAD --pretty=format:"%B"`
      : 'git log --pretty=format:"%B"';
    
    const logResult = executeGitCommand(gitLogCommand, true);
    if (!logResult.success || !logResult.output.trim()) {
      console.log(colors.BYellow(`${SYMBOLS.INFO} No commits found since last release`));
      return;
    }
    
    // Filter out irrelevant commits and strip JIRA tickets (matching bash script logic)
    const commitMessages = logResult.output
      ...
      if (firstLine.startsWith('- ')) {
        ...
        return lines.map((line, index) => {
          ...
          if (index === 0) {
            ...
            return `\t- ${trimmed}`; // Indented bullets for body
          }
          return '';
    ...
    // Also create/update internal changelog (matching bash script logic)
    await updateInternalChangelog(newVersion, commitMessages, date, fs);
 ...
 * @param {string} newVersion - New version number
 * @param {Array} commitMessages - Raw commit messages
 * @param {string} date - Formatted date string
 * @param {Object} fs - File system module
 * @returns {Promise<void>}
 */
async function updateInternalChangelog(newVersion, commitMessages, date, fs) {
  const internalChangelogPath = process.env.MAIASS_CHANGELOG_INTERNAL_NAME || 'CHANGELOG_internal.md';
  
  try {
    // Format internal changelog with raw commit messages (no bullet processing)
    const rawCommits = commitMessages.map(commit => {
      // Just clean up and indent, no bullet formatting
      return commit.split('\n')
        .map(line => line.trim())
        .filter(line => line)
    ...
    // Use longer date format for internal changelog (matching bash script)
    ...
    // Handle existing internal changelog or create new one
    let updatedContent;
    try {
      await fs.access(internalChangelogPath);
      const currentContent = await fs.readFile(internalChangelogPath, 'utf8');
      
      // Check if we need to update existing version entry (same logic as main changelog)
      const lines = currentContent.split('\n');
      const firstLine = lines[0] || '';
      const secondLine = lines[1] || '';
      ...
      if (currentVersionMatch) {
        ...
        if (currentMajorMinor === newMajorMinor && secondLine === longDate) {
          // Same version and date - replace the entry
          const restOfContent = lines.slice(3).join('\n');
          updatedContent = `## ${newVersion}\n${longDate}\n\n${rawCommits}\n\n${restOfContent}`;
        } else {
      ...
      // Internal changelog doesn't exist - create new one
      updatedContent = `## ${newVersion}\n${longDate}\n\n${rawCommits}\n`;
    }
    
    // Write updated internal changelog
    await fs.writeFile(internalChangelogPath, updatedContent, 'utf8');
    console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Updated ${internalChangelogPath}`));
...
async function handleVersionManagement(branchInfo, mergeResult, options = {}) {
  ...
  try {
    ...
    let shouldTag = tag; // CLI flag takes precedence
