...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    // Get the actual latest git tag (since we don't tag every patch release)
    const latestTagResult = executeGitCommand('git describe --tags --abbrev=0', true);
    let lastTag = latestTagResult.success ? latestTagResult.output.trim() : null;
    
    // If no git tag exists, use a fallback to get all commits
    if (!lastTag) {
      log.warning(SYMBOLS.INFO, 'No git tags found - getting all commits');
      lastTag = null; // This will make git log get all commits
    } else {
      log.blue(SYMBOLS.GEAR, `Getting commits since last git tag: ${lastTag}`);
    }
    ...
    const internalGitLogCommand = lastTag 
      ? `git log "${lastTag}"..HEAD --pretty=format:"(%an) %s"` 
...
/**
 * Check for MAIASS version updates from GitHub releases
...
async function checkForUpdates(currentVersion) {
  try {
    ...
    const response = await fetch('https://api.github.com/repos/vsmash/maiass/releases/latest', {
      headers: {
