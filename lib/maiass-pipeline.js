...
async function handleCommitWorkflow(branchInfo, options = {}) {
  const { commitsOnly = false, autoStage = false, silent = false, dryRun = false } = options;
...
export async function runMaiassPipeline(options = {}) {
  ...
  try {
    ...
    const commitResult = await handleCommitWorkflow(branchInfo, { commitsOnly, autoStage, silent, dryRun });
