...
async function handleMergeToDevelop(branchInfo, commitResult, options = {}) {
  const { force = false, silent = false, originalGitInfo = null, autoSwitch = true } = options;
  ...
  if (!force) {
    ...
    if (silent) {
      ...
      reply = await getSingleCharInput(`Merge to ${developBranch} for version management? [Y/n] `);
    }
