...
async function handleMergeToDevelop(branchInfo, commitResult, options = {}) {
  ...
  if (remoteExists()) {
    logger.info(SYMBOLS.GEAR, 'Pulling latest changes from remote...');
...
async function handleVersionManagement(branchInfo, mergeResult, options = {}) {
  ...
  try {
    ...
    if (hasRemote) {
      logger.info(SYMBOLS.GEAR, `Pushing release branch to remote...`);
    ...
    if (hasRemote) {
      logger.info(SYMBOLS.GEAR, `Pushing updated ${developBranch}...`);
