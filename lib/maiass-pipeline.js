...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    if (commitMessages.length === 0) {
      log.warning(SYMBOLS.INFO, 'No relevant commits found for changelog - skipping changelog update');
      // Skip changelog update for pure version bumps with no meaningful commits
      return;
    } else {
    ...
    // If we still don't have formatted commits, use default
    if (!formattedCommits) {
      formattedCommits = '- Version bump';
    }
...
async function updateInternalChangelog(newVersion, internalCommitMessages, date) {
  ...
  try {
    if (!internalCommitMessages || internalCommitMessages.length === 0) {
      log.warning(SYMBOLS.INFO, 'No commits for internal changelog - skipping internal changelog update');
      // Skip internal changelog update for pure version bumps with no meaningful commits
      return;
    }
...
export async function runMaiassPipeline(options = {}) {
  ...
  try {
    ...
    logger.info(SYMBOLS.INFO, `Final branch: ${getCurrentBranch()}`);
    logger.info(SYMBOLS.INFO, `Final version: ${versionResult.version || '(none)'}`);
    logger.info(SYMBOLS.INFO, `Workflow used: ${versionResult.workflow || 'unknown'}`);
    return {
      success: true,
      branchInfo,
      commitResult,
    ...
    const { writeDebugFile, getDebugSession } = await import('./logger.js');
    const debugFile = writeDebugFile();
    if (debugFile) {
      logger.error(SYMBOLS.INFO, `Debug information written to: ${debugFile}`);
    }
