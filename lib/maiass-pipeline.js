...
async function updateChangelog(newVersion) {
  ...
  try {
    ...
    if (!lastTag) {
      console.log(colors.BYellow(`${SYMBOLS.INFO} No previous release tag found - skipping changelog update`));
      return;
    }
    ...
    await updateInternalChangelog(newVersion, internalCommitMessages, date, fs);
...
async function updateInternalChangelog(newVersion, commitMessages, date, fs) {
  ...
  try {
    ...
    // Create new entry for internal changelog
