...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    log.blue(SYMBOLS.INFO, `Getting commits since last tag: ${lastTag}`);
...
async function switchToBranch(branchName) {
  logger.info(SYMBOLS.INFO, `Switching to ${branchName} branch...`);
...
async function validateAndHandleBranching(options = {}) {
  ...
  logger.header(SYMBOLS.INFO, 'Branch Detection and Validation');
...
async function handleMergeToDevelop(branchInfo, commitResult, options = {}) {
  ...
  logger.info(SYMBOLS.INFO, `Merging ${currentBranch} into ${developBranch}...`);
  ...
  if (remoteExists()) {
    log.blue(SYMBOLS.INFO, 'Pulling latest changes from remote...');
...
async function handleVersionManagement(branchInfo, mergeResult, options = {}) {
  ...
  try {
    ...
    // change this to use logger
    logger.info(SYMBOLS.INFO, 'Committing version changes...');
    ...
    if (hasRemote) {
      logger.info(SYMBOLS.INFO, `Pushing release branch to remote...`);
    ...
    logger.info(SYMBOLS.INFO, `Merging release back to ${developBranch}...`);
    ...
    if (hasRemote) {
      logger.info(SYMBOLS.INFO, `Pulling latest ${developBranch}...`);
      const pullResult = executeGitCommand(`git pull origin ${developBranch}`);
      if (!pullResult.success) {
    ...
    if (hasRemote) {
      logger.info(SYMBOLS.INFO, `Pushing updated ${developBranch}...`);
...
export async function runMaiassPipeline(options = {}) {
  ...
  try {
    ...
    // change this to use logger
    logger.info(SYMBOLS.INFO, 'Phase 1: Branch Detection and Validation');
