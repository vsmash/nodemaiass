...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    // Get commit messages with full info for processing both changelogs
    const logResult = executeGitCommand(`git log "${lastTag}"..HEAD --pretty=format:"%B|||%s|||%an"`, true);
    if (!logResult.success || !logResult.output.trim()) {
      console.log(colors.BYellow(`${SYMBOLS.INFO} No commits found since last release`));
      return;
    }
    
    // Parse commits with full body, subject, and author
    const rawCommits = logResult.output
      .split('\n\n') // Split by double newlines (commit boundaries)
      .map(commit => {
        const parts = commit.trim().split('|||');
        if (parts.length !== 3) return null;
        
        const [fullBody, subject, author] = parts;
        return {
    ...
    const relevantCommits = rawCommits.filter(commit => {
      const firstLine = commit.subject.toLowerCase();
    ...
    // Process commits for main changelog (strip JIRA tickets)
    const mainCommits = relevantCommits.map(commit => {
      // Strip JIRA ticket numbers from commit messages
      return commit.fullBody.replace(/^[A-Z]+-[0-9]+ /gm, '');
    });
    // Process commits for internal changelog (keep JIRA tickets, add author)
    const internalCommits = relevantCommits.map(commit => {
      return `${commit.subject} (${commit.author})`;
    });
    
    // Format commit messages with proper bullets (matching bash script logic)
    const formattedCommits = mainCommits.map(commit => {
      ...
      if (firstLine.startsWith('- ')) {
        ...
        return lines.map((line, index) => {
          ...
          if (index === 0) {
            ...
            if (trimmed.startsWith('- ')) {
              return `\t${trimmed}`; // Indent existing bullet
            } else {
              return `\t${trimmed}`; // Just indent, no bullet for body lines
            }
          }
          return '';
    ...
    if (formattedCommits.length === 0) {
      console.log(colors.BYellow(`${SYMBOLS.INFO} No commits to add to changelog`));
      return;
    }
    ...
    await updateInternalChangelog(newVersion, internalCommits, date);
...
/**
 * Update internal changelog with commit messages including author info
 * @param {string} newVersion - New version number
 * @param {Array} internalCommits - Array of commit messages with author info
 * @param {string} date - Formatted date string
 */
async function updateInternalChangelog(newVersion, internalCommits, date) {
  ...
  try {
    if (!internalCommits || internalCommits.length === 0) {
      console.log(colors.BYellow(`${SYMBOLS.INFO} No commits for internal changelog`));
      return;
    }
    
    // Format commit messages for internal changelog (simple bullet list with author info)
    const formattedCommits = internalCommits.map(commit => `- ${commit}`).join('\n');
