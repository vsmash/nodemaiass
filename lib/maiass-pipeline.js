...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    const mainLogResult = executeGitCommand(`git log "${lastTag}"..HEAD --pretty=format:"%B==(%an)"`, true);
    ...
    const internalCommitMessages = mainLogResult.success ? mainLogResult.output
      .split('\n')
      .filter(line => {
