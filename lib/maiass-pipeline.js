...
async function handleCommitWorkflow(branchInfo, options = {}) {
  const { commitsOnly = false, autoStage = false, silent = false, providedMessage = null } = options;
...
export async function runMaiassPipeline(options = {}) {
  ...
  try {
    ...
    const commitResult = await handleCommitWorkflow(branchInfo, { commitsOnly, autoStage, silent, providedMessage });
