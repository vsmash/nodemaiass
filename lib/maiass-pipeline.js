...
async function updateChangelog(newVersion) {
  ...
  try {
    ...
    const formattedCommits = commitMessages.map(commit => {
      ...
      // Check if it's already an AI commit with bullets
      if (firstLine.startsWith('- ')) {
        // AI commit - keep main bullet, indent sub-bullets
        return lines.map((line, index) => {
          const trimmed = line.trim();
          if (index === 0) {
            return trimmed; // Keep main bullet as-is
          } else if (trimmed.startsWith('- ')) {
            return `\t${trimmed}`; // Indent existing sub-bullets
          } else if (trimmed) {
            return `\t${trimmed}`; // Just indent body lines (don't add extra bullets)
          }
          return '';
        }).filter(line => line).join('\n');
      } else {
        // Manual commit - add bullets
        return lines.map((line, index) => {
          const trimmed = line.trim();
          if (index === 0) {
