...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    const internalLogResult = executeGitCommand(`git log "${lastTag}"..HEAD --pretty=format:"%s (%an)"`, true);
    if (!internalLogResult.success) {
      console.log(colors.BYellow(`${SYMBOLS.WARNING} Failed to get commits for internal changelog`));
    }
    
    const rawInternalLogResult = executeGitCommand(
      `git log "${lastTag}"..HEAD --pretty=format:"%B%n==END==%an"`,
      true
    );
    ...
    const internalCommitMessages = rawInternalLogResult.success
    ? rawInternalLogResult.output.split('\n==END==\n').map(chunk => {
        const lines = chunk.trim().split('\n');
        const author = lines.pop(); // last line is author
      ...
      .filter(({ commit }) => {
        if (!commit) return false;
        const firstLine = commit.split('\n')[0].toLowerCase();
        return !/^(merge branch|bump|ncl|version bump|fixing merge conflicts)/.test(firstLine);
      })
    : [];
    ...
    const formattedInternalCommits = internalCommitMessages.map(({ commit, author }) => {
      const lines = commit.split('\n').filter(Boolean);
      if (lines.length === 0) return '';
    
      const firstLine = lines[0].trim();
      const jiraPrefixMatch = firstLine.match(/^([A-Z]+-[0-9]+)\s+(.*)/);
      let jiraPrefix = '';
      let subject = firstLine;
    
      if (jiraPrefixMatch) {
        jiraPrefix = jiraPrefixMatch[1];
        subject = jiraPrefixMatch[2];
      }
    
      const header = `- ${jiraPrefix ? jiraPrefix + ' ' : ''}${subject} (${author})`;
...
async function updateInternalChangelog(newVersion, internalCommitMessages, date) {
  ...
  try {
    ...
    const formattedCommits = internalCommitMessages.map(commit => `- ${commit}`).join('\n');
    ...
    await fs.writeFile(internalChangelogPath, `## ${newVersion}\n${date}\n\n${formattedInternalCommits}\n`, 'utf8');
