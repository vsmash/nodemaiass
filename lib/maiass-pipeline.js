...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    // Create date string with weekday for internal changelog (format: "23 November 2025 (Sunday)")
    const now = new Date();
    ...
    // Also create/update internal changelog with weekday format
    await updateInternalChangelog(newVersion, internalCommitMessages, dateWithWeekday);
  ...
  try {
    ...
    try {
      ...
      if (currentVersionMatch) {
        ...
        // Normalize date strings for comparison (extract just the date part, ignore day names)
        const normalizeDate = (dateStr) => {
          // Extract date pattern like "24 November 2025" from various formats
          const match = dateStr.match(/(\d{1,2}\s+\w+\s+\d{4})/);
          return match ? match[1] : dateStr;
        };
        
        const normalizedCurrentDate = normalizeDate(secondLine);
        const normalizedNewDate = normalizeDate(date);
        
        if (currentMajorMinor === newMajorMinor && normalizedCurrentDate === normalizedNewDate) {
          // Same version and date - append to existing entry instead of replacing
