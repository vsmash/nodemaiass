...
async function updateChangelog(newVersion) {
  ...
  try {
    ...
    console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Updated ${changelogPath}`));
  } catch (error) {
    console.log(colors.BYellow(`${SYMBOLS.WARNING} No changelog found or failed to update: ${changelogPath}`));
  }
}
...
async function handleVersionManagement(branchInfo, mergeResult, options = {}) {
  ...
  try {
    ...
    const { success: branchCreated } = await executeGitCommand(`git checkout -b ${releaseBranch}`);
    if (!branchCreated) {
      console.error(colors.Red(`${SYMBOLS.CROSS} Failed to create release branch`));
    ...
    const { success: committed } = await executeGitCommand(`git commit -m "Bumped version to ${newVersion}"`);
    if (!committed) {
      console.error(colors.Red(`${SYMBOLS.CROSS} Failed to commit version changes`));
    ...
    if (tag) {
      ...
      const { success: tagged } = await executeGitCommand(`git tag -a ${newVersion} -m "Release version ${newVersion}"`);
      if (tagged) {
        console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Tagged release as ${newVersion}`));
      } else {
        console.log(colors.BYellow(`${SYMBOLS.WARNING} Failed to create tag (continuing anyway)`));
      }
    }
    ...
    if (hasRemote) {
      ...
      const { success: pushed } = await executeGitCommand(`git push --set-upstream origin ${releaseBranch}`);
      if (pushed) {
        console.log(colors.BGreen(`${SYMBOLS.CHECKMARK} Pushed ${releaseBranch} to remote`));
      } else {
        console.log(colors.BYellow(`${SYMBOLS.WARNING} Failed to push release branch (continuing anyway)`));
      }
    }
    
    // Switch back to develop and merge release
    console.log(colors.BBlue(`${SYMBOLS.INFO} Merging release back to ${developBranch}...`));
    const { success: switchedToDevelop } = await executeGitCommand(`git checkout ${developBranch}`);
    if (!switchedToDevelop) {
      console.error(colors.Red(`${SYMBOLS.CROSS} Failed to switch back to ${developBranch}`));
    ...
    if (hasRemote) {
      ...
      await executeGitCommand(`git pull origin ${developBranch}`);
    }
    
    // Merge release branch back to develop
    const { success: merged } = await executeGitCommand(`git merge --no-ff ${releaseBranch}`);
    if (!merged) {
      console.error(colors.Red(`${SYMBOLS.CROSS} Failed to merge ${releaseBranch} back to ${developBranch}`));
