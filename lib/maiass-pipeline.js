  ...
  try {
    ...
    const internalCommitMessages = internalLogResult.success ? internalLogResult.output
      ...
      .filter(line => {
        ...
        const dashAuthor = !/^-\s*\([^)]+\)\s*(ncl|merge|bump|bumping|fixing merge conflicts|version bump|merged)\b/i.test(line.trim());
        const withSha = !/^([a-f0-9]+|\-\s*\([^)]+\))\s*(ncl|merge|bump|bumping|fixing merge conflicts|version bump|merged)\b/i.test(line.trim());
        const shouldInclude = !/^[a-f0-9]+\s*(\([^)]+\)\s*)?(ncl|merge|bump|bumping|fixing merge conflicts|version bump|merged)\b/i.test(lowerLine);
