...
async function updateChangelog(newVersion, currentVersion) {
  ...
  try {
    ...
    if (commitMessages.length === 0) {
      log.warning(SYMBOLS.INFO, 'No relevant commits found for changelog - updating version only');
      // Still update changelog with version, but skip commit entries
...
async function updateInternalChangelog(newVersion, internalCommitMessages, date) {
  ...
  try {
    ...
    if (!internalCommitMessages || internalCommitMessages.length === 0) {
      log.warning(SYMBOLS.INFO, 'No commits for internal changelog - updating version only');
      // Still update internal changelog with version, but use simple entry
      formattedCommits = '- Version bump';
    } else {
