...
/**
 * Flags accepted by `maiass config`.
 * Consumed by the global flag validator in maiass.mjs (MAI-43) so subcommand
 * flags are not rejected upstream, while typos on these flags still ARE
 * rejected with a useful error message (closes the MAI-39 MINOR).
 */
export const FLAGS = [
  '--global',
  '--project',
