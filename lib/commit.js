...
async function handleStagedCommit(gitInfo, options = {}) {
  const { silent = false, dryRun = false, forceStaged = false } = options;
...
export async function commitThis(options = {}) {
  const { autoStage = false, commitsOnly = false, silent = false, dryRun = false } = options;
    ...
    if (!autoStage) {
      ...
      if (reply === 'y') {
        if (dryRun) {
          ...
          return await handleStagedCommit(gitInfo, { silent, dryRun, forceStaged: true });
        }
        // Stage all changes
        ...
        return await handleStagedCommit(updatedGitInfo, { silent, dryRun });
      } else {
        // Check if there are staged changes to commit
        if (status.stagedCount > 0) {
          log.info(SYMBOLS.INFO, 'Proceeding with staged changes only');
          return await handleStagedCommit(gitInfo, { silent, dryRun });
        }
