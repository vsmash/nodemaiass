...
async function getAICommitSuggestion(gitInfo) {
  ...
  try {
    ...
    // Compose exclusion patterns for git diff
    const excludeArgs = [
      `:(exclude)${changelogName}`,
      `:(exclude)${internalChangelogName}`
    ];
    // Get git diff for staged changes, excluding changelogs
    let gitDiff = executeGitCommand(`git diff --cached -- ${excludeArgs.join(' ')}`, true);
    if (!gitDiff) {
      return null;
    }
    ...
    if (debugMode) {
      ...
      log.debug(SYMBOLS.INFO, `[MAIASS DEBUG] Model: ${openaiModel}`);
      log.debug(SYMBOLS.INFO, `[MAIASS DEBUG] Temperature: ${openaiTemperature}`);
