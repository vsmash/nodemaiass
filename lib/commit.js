...
async function handleStagedCommit(gitInfo, options = {}) {
  ...
  try {
    ...
    if (remoteExists('origin')) {
      let reply;
      if (silent) {
        log.prompt('Do you want to push this commit to remote? [y/N] y');
        reply = 'y';
      } else {
...
export async function commitThis(options = {}) {
  ...
  if (status.unstagedCount > 0 || status.untrackedCount > 0) {
    ...
    if (status.unstagedCount > 0) {
      log.critical('', `  Unstaged: ${status.unstagedCount} file${status.unstagedCount === 1 ? '' : 's'}`);
    }
    if (status.untrackedCount > 0) {
      log.critical('', `  Untracked: ${status.untrackedCount} file${status.untrackedCount === 1 ? '' : 's'}`);
    }
    
    if (!autoStage) {
      let reply;
      if (silent) {
        log.prompt(`Do you want to stage and commit them? [y/N] y`);
        reply = 'y';
      } else {
