...
async function getAICommitSuggestion(gitInfo) {
  ...
  const openaiToken = process.env.MAIASS_OPENAI_TOKEN;
  const openaiEndpoint = process.env.MAIASS_OPENAI_ENDPOINT || 'https://api.openai.com/v1/chat/completions';
  const openaiModel = process.env.MAIASS_OPENAI_MODEL || 'gpt-3.5-turbo';
  const openaiTemperature = parseFloat(process.env.MAIASS_OPENAI_TEMPERATURE || '0.7');
  const commitMessageStyle = process.env.MAIASS_OPENAI_COMMIT_MESSAGE_STYLE || 'bullet';
  ...
  try {
    ...
    const gitDiff = executeGitCommand('git diff --cached', true);
    if (!gitDiff) {
      return null;
    }
 ...
 */
async function getCommitMessage(gitInfo) {
  const openaiMode = process.env.MAIASS_OPENAI_MODE || 'ask';
  const openaiToken = process.env.MAIASS_OPENAI_TOKEN;
  ...
  switch (openaiMode) {
    case 'ask':
      if (openaiToken) {
        const reply = await getSingleCharInput(colors.BYellow('Would you like to use AI to suggest a commit message? [y/N] '));
        useAI = reply === 'y';
      }
      break;
    case 'autosuggest':
  ...
  if (useAI) {
    ...
    if (aiSuggestion) {
      ...
      const reply = await getSingleCharInput(colors.BCyan('Use this AI suggestion? [Y/n/e=edit] '));
      ...
      switch (reply) {
        ...
        case 'e':
          ...
          return editedMessage || aiSuggestion;
        default:
 ...
 */
async function handleStagedCommit(gitInfo) {
  ...
  // Get commit message
  const commitMessage = await getCommitMessage(gitInfo);
  if (!commitMessage) {
  ...
  try {
    ...
    const result = executeGitCommand(commitCommand, silent);
    ...
    if (remoteExists('origin')) {
      const reply = await getSingleCharInput(colors.BYellow('Do you want to push this commit to remote? [y/N] '));
...
export async function commitThis(options = {}) {
  const { autoStage = false, commitsOnly = false } = options;
  ...
  if (status.unstagedCount > 0 || status.untrackedCount > 0) {
    ...
    if (!autoStage) {
      const reply = await getSingleCharInput(colors.BYellow(`Do you want to ${colors.BRed('stage and commit')} them? [y/N] `));
      ...
      if (reply === 'y') {
        ...
        return await handleStagedCommit(updatedGitInfo);
      } else {
        // Check if there are staged changes to commit
        if (status.stagedCount > 0) {
          return await handleStagedCommit(gitInfo);
        }
