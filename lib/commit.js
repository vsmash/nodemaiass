...
export async function commitThis(options = {}) {
  const { autoStage = false, commitsOnly = false, silent = false, providedMessage = null } = options;
  ...
  if (status.unstagedCount > 0 || status.untrackedCount > 0) {
    if (!autoStage) {
      ...
      if (reply === 'y') {
        ...
        return await handleStagedCommit(updatedGitInfo, { silent, providedMessage });
      } else {
        // Check if there are staged changes to commit
        if (status.stagedCount > 0) {
          log.info(SYMBOLS.INFO, 'Proceeding with staged changes only');
          return await handleStagedCommit(gitInfo, { silent, providedMessage });
        }
