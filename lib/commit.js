...
async function handleStagedCommit(gitInfo, options = {}) {
  ...
  try {
    ...
    if (remoteExists('origin')) {
      ...
      if (reply === 'y') {
        // if verbosity is debug, show the push command
        if (verbosity === 'debug'||debug== true) {
          const pushResult = executeGitCommand(`git push --set-upstream origin '${gitInfo.branch}'`, false);
        }else{
          const pushResult = executeGitCommand(`git push --set-upstream origin '${gitInfo.branch}'`, true);
        }
        if (pushResult !== null) {
